...
#ifndef UWORLD_DECRYPTION_H
#define UWORLD_DECRYPTION_H
...
uint64_t ReadWorld()
{
	__try
	{
		...
		uint64_t key = read<uint64_t>(BaseAddr + offsets::uworld_key);
		//std::printf("key %p \n", key);
...
#pragma pack(pop)
		const auto state = read<State>(BaseAddr + offsets::uworld_state);
		// std::printf("state %p \n", state.Keys);
...
uintptr_t decrypt_uworld() {
	const auto key = read<uintptr_t>(BaseAddr + offsets::uworld_key);
	struct State
	{
		uintptr_t keys[7];
	};
	...
	return read<uintptr_t>(uworld);
}

#endif // UWORLD_DECRYPTION_H
#pragma warning(pop)